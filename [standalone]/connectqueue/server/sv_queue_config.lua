...
Config.Language = {
	...
	steam = "\xE2\x9D\x97 [Queue] Error: Steam must be enabled."
}
...
QBCore.Commands.Add("addpriority", "Give queue priority", {{name="id", help="player ID"}, {name="priority", help="Priority level"}}, true, function(source, args)
	...
	if Player ~= nil then
        ...
        TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "You have prioritized " .. GetPlayerName(Player.PlayerData.source) .." ("..level..")")
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Player is not online!")
	end
end, "god")

QBCore.Commands.Add("removepriority", "Remove priority from someone", {{name="id", help="player ID"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
        RemovePriority(Player.PlayerData.source)
        TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "You have unprioritised " .. GetPlayerName(Player.PlayerData.source))
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Player is not online!")
	end
end, "god")
