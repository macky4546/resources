...
RegisterServerEvent('police:server:updateObject')
AddEventHandler('police:server:updateObject', function(objectId, coords, rotation)
    TriggerClientEvent("police:client:updateObject", -1, objectId, coords, rotation)
end)
...
QBCore.Commands.Add("editobject", "Pas een object rotatie/positie aan (aan/uit)", {}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:editObject", source)
    end
end)
