        ...
        if isLoggedIn then
            for k, v in pairs(Config.RentalPoints) do
                ...
                if dist < 30 then
                    ...
                    if dist < 1 then
                        DrawText3Ds(Config.RentalPoints[k]["coords"][1]["x"], Config.RentalPoints[k]["coords"][1]["y"], Config.RentalPoints[k]["coords"][1]["z"] + 0.35, '~g~E~w~ - To rent a vehicle')
            ...
            for k, v in pairs(Config.DeliveryPoints) do
                ...
                if dist < 30 then
                    ...
                    if dist < 1 then
                        DrawText3Ds(Config.DeliveryPoints[k]["coords"]["x"], Config.DeliveryPoints[k]["coords"]["y"], Config.DeliveryPoints[k]["coords"]["z"] + 0.35, '~g~E~w~ - hand in vehicle')
                        if IsControlJustPressed(0, Keys["E"]) then
                            ReturnVehicle()
                            Menu.hidden = not Menu.hidden
                            CurrentRentalPoint = k
...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.DeliveryPoints) do
        ...
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentSubstringPlayerName("Rental - Drop off point")
...
ReturnVehicle = function()
    ...
    QBCore.Functions.Notify('You have no deposit outstanding.', 'error')
end
