...
AddEventHandler('beginJailRehab', function(isInRehabSent)
    ...
    QBCore.Functions.Notify("You are on a mental hold.", "error")
    Citizen.Wait(1000)
    ...
    if isInRehab then
        while isInRehab do
            ...
            if not inside then
                ...
                QBCore.Functions.Notify("The orderly have placed you back into the facility for protection.", "error")
                SetEntityCoords(PlayerPedId(), groundCoords[placement][1],groundCoords[placement][2],groundCoords[placement][3]) 
            end
    ...
    QBCore.Functions.Notify("You were removed from Mental health care.", "error")
