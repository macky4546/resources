...
function GetSociety(name)
    local result = QBCore.Functions.ExecuteSql(true, "SELECT * FROM society WHERE name = '"..name.."' ")
...
AddEventHandler('society:server:WithdrawMoney', function(a, n)
    ...
    if amount < sMoney then
        QBCore.Functions.ExecuteSql(false, "UPDATE society SET money = '"..sMoney - amount.."' WHERE name = '"..n.."'")
        player.Functions.AddMoney('cash', amount, "Society Withdraw")
    else
        TriggerClientEvent('QBCore:Notify', src, "You don\'t have enough money in society")
    end
end)
...
AddEventHandler('ps-society:server:DepositSQL', function(amount, name) -- DONT TUCH THIS..
    ...
    QBCore.Functions.ExecuteSql(false, "UPDATE society SET money = '"..deposit.."' WHERE name = '"..name.."'")
end)
