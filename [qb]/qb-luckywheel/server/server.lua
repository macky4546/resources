...
RegisterServerEvent('esx_tpnrp_luckywheel:carRedeem')
AddEventHandler('esx_tpnrp_luckywheel:carRedeem', function(vehicleProps)
    local _source = source
    veh = 'coquette4'
    ...
    while getPlate do
        ...
        if plateAvailable == nil then
            ...
            getPlate = false
            print('Casino Car Won: '..testplate)
        end
    end
    local vehiclePropsjson = json.encode(vehicleProps)
    if car then
        car = false 
        TriggerClientEvent('QBCore:Notify', _source, "You won a car!", "success")
        TriggerClientEvent('esx_tpnrp_luckywheel:winCarEmail', _source)
        QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_vehicles` (`steam`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `garage`, `state`) VALUES ('"..xPlayer.PlayerData.steam.."', '"..xPlayer.PlayerData.citizenid.."', '"..veh.."', '"..GetHashKey(veh).."', '"..vehiclePropsjson.."', '"..vehicleProps.plate.."', '"..storedGarage.."', '"..stateVehicle.."')")
    else
