...
QBCore.Commands.Add("setlawyer", "Register someone as a lawyer", {{name="id", help="Player ID"}}, true, function(source, args)
...
QBCore.Commands.Add("removelawyer", "Remove someone as a lawyer", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local playerId = tonumber(args[1])
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if Player.PlayerData.job.name == "judge" then
        if OtherPlayer ~= nil then 
            ...
            TriggerClientEvent("QBCore:Notify", source, "You have " .. OtherPlayer.PlayerData.charinfo.firstname .. " " .. OtherPlayer.PlayerData.charinfo.lastname .. " fired as lawyer")
        else
            TriggerClientEvent("QBCore:Notify", source, "Person is not present..", "error")
        end
    else
