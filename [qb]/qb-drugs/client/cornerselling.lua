...
local policeMessage = {
    "Suspicious situation on ",
    "Possible drug sales at ",
}
    ...
    QBCore.Functions.TriggerCallback('qb-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            ...
            if not cornerselling then
                cornerselling = true
                QBCore.Functions.Notify('Corner selling position turned on')
            ...
            QBCore.Functions.Notify('You have no drugs on you', 'error')
        end
    end)
end)

function toFarAway()
    QBCore.Functions.Notify('Youre walking too much.. start again!', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
        ...
        if stealingPed ~= nil and stealData ~= nil then
            if IsEntityDead(stealingPed) then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, pedpos.x, pedpos.y, pedpos.z, true) < 1.5) then
                    DrawText3D(pedpos.x, pedpos.y, pedpos.z, "[E] Take things back")
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                if pedDist < 1.5 then
                    QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for $'..randomPrice..'? / ~g~G~w~ Reject offer')
