                        ...
                        if dist < 30 then
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    DrawText3Ds(Config.Locations[case]["coords"]["x"], Config.Locations[case]["coords"]["y"], Config.Locations[case]["coords"]["z"], '[E] smashing showcase')
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                ...
                                                QBCore.Functions.Notify('There are not enough agents...', 'error')
                                            end                
                                        end)
                                    end
                                end
                            ...
                            if storeDist < 2 then
                                if not firstAlarm then
                                    if validWeapon() then
                                        TriggerServerEvent('qb-jewellery:server:PoliceAlertMessage', 'There is a suspicious situation at Vangelico Jeweler', pos, true)
                                        firstAlarm = true
                                    end
                                end
                            end
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        QBCore.Functions.Notify("You hurt yourself on the glass..", "error")
    end

    QBCore.Functions.Progressbar("smash_vitrine", "Smashing showcase..", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        TriggerServerEvent('qb-jewellery:server:PoliceAlertMessage', "There is a robbery going on at Vangelico Jeweler", plyCoords, false)
...
AddEventHandler('qb-jewellery:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
        ...
        AddTextComponentString("112 -Suspicious situation jeweler")
...
Citizen.CreateThread(function()
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Vangelico Jewelry store")
